...
img_size_x = int(os.environ['DIE_SIZE_X'])
img_size_y = int(os.environ['DIE_SIZE_Y'])
...
for key,val in wire_bond_map.items():
  ps_print_box(fid, val['Pad Name'], val['Net Name'], (float(val['Die X']) - (float(val['Width'] ) / 2.0)) * img_scale + img_size_x // 2,
                                                       (float(val['Die Y']) - (float(val['Height']) / 2.0)) * img_scale + img_size_y // 2,
                                                       (float(val['Die X']) + (float(val['Width'] ) / 2.0)) * img_scale + img_size_x // 2,
                                                       (float(val['Die Y']) + (float(val['Height']) / 2.0)) * img_scale + img_size_y // 2,
                                                       0)
